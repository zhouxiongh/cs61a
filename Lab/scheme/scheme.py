...
class PrimitiveProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            ...
            raise SchemeError("worong number of arguments were passed")
        # END PROBLEM 4
...
def make_let_frame(bindings, env):
    ...
    # BEGIN PROBLEM 15
    "*** YOUR CODE HERE ***"
    formals, expr = nil, nil
    while bindings is not nil:
        check_form(bindings.first, 2, 2)
        formals = Pair(bindings.first.first, formals)
        expr = Pair(eval_all(bindings.first.second, env), expr)
        bindings = bindings.second
    check_formals(formals)
    return env.make_child_frame(formals, expr)
    # END PROBLEM 15
