...
def pingpong(n):
    ...
    def helper_next(k, p, up):
        if k == n:
            return p
        if up:
            return helper_switch(k+1, p + 1, up)
        else:
            return helper_switch(k+1, p - 1, up)

    def helper_switch(k, p, up):
        if k % 7 == 0 or has_seven(k):
            return helper_next(k, p, not up)
        else:
            return helper_next(k, p, up)
    return helper_next(1, 1, True)
def has_seven(k):
